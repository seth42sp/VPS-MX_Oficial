...
sh_ver="1.3.1"
github="raw.githubusercontent.com/cx9208/Linux-NetSpeed/main"
...
startbbrmod_nanqinlang(){
	...
	if [[ "${release}" == "centos" ]]; then
		yum install -y make gcc
		mkdir bbrmod && cd bbrmod
		wget -N --no-check-certificate https://raw.githubusercontent.com/chiakge/Linux-NetSpeed/main/bbr/centos/tcp_nanqinlang.c
		...
		mkdir bbrmod && cd bbrmod
		wget -N --no-check-certificate https://raw.githubusercontent.com/chiakge/Linux-NetSpeed/main/bbr/tcp_nanqinlang.c
		echo "obj-m := tcp_nanqinlang.o" > Makefile
		make -C /lib/modules/$(uname -r)/build M=`pwd` modules CC=/usr/bin/gcc-4.9
...
startlotserver(){
	...
	bash <(wget --no-check-certificate -qO- https://github.com/MoeClub/lotServer/raw/main/Install.sh) install
	start_menu
}
...
remove_all(){
	...
	if [[ -e /appex/bin/lotServer.sh ]]; then
		bash <(wget --no-check-certificate -qO- https://github.com/MoeClub/lotServer/raw/main/Install.sh) uninstall
	fi
	clear
...
msg -bar
msg -tit
echo -e "    TCP Aceleración (BBR/Plus) VPS-MX By @seth42sp\n                         ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
$(msg -bar)
